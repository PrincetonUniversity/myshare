...
if __name__ == "__main__":
    ...
    for node_id in mytree.tree.expand_tree():
        if node_id.endswith(f" ({args.user})"):
            ...
            if args.verbose:
                if False:
                    print("First, be aware that the Shares of an account sets the maximum Usage when the")
                    down = "\u2193"
                    print(f"cluster is busy. The table below ({down}) shows the accounts sorted by Shares:\n")
                    print(mytree.get_descendants_table(node_id=node_id_dept,
                                                       decimals=1,
                                                       sort_by="RawShares",
                                                       tabbing=-1,
